...
_Msh_qV_sngQuote() {
	...
	case ${_Msh_qV_VAL_sng} in
	( *\' )	# On most shells, non-whitespace IFS discards a final empty field, so add one.
		thisshellhas QRK_IFSFINAL || _Msh_qV_VAL_sng=${_Msh_qV_VAL_sng}\' ;;
	esac
	_Msh_qV=
	push IFS -f; set -f
	IFS="'"; for _Msh_qV_C in ${_Msh_qV_VAL_sng}; do IFS=
		...
		( *[!$SHELLSAFECHARS]* )
			...
			( *\'* )
				...
				( *[\"\$\`\\]* )
					...
					_Msh_qV_VAL_sng=${_Msh_qV_VAL}
					_Msh_qV_dblQuote
					_Msh_qV_sngQuote
					let "${#_Msh_qV_VAL_sng} < ${#_Msh_qV_VAL}" && _Msh_qV_VAL=${_Msh_qV_VAL_sng}
					unset -v _Msh_qV_VAL_sng ;;
