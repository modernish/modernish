...
The `--version` and `--help` options output the respective information and exit.
...
shells with `BUG_NOOCTAL`) a leading `0` denotes an octal number.
...
* `BUG_ALIASPOSX`: Running any command "foo" in POSIX mode like
  ...
  **Note:** Modernish 0.16.x always enables POSIX mode for bash at init time,
  which avoids triggering this bug. Be sure not to `unset POSIXLY_CORRECT`.
...
* `BUG_PP_HERE_SP`: `$*` in a here-document always uses a space as the output
  ...
  Found on: ksh93 before 93u+m/1.0.11; old bash versions
