...
tests if 'this shell has' a particular reserved word or builtin command.

These IDs are used by library functions to conveniently work around bugs or
take advantage of special features not all shells have. For instance, `match
-E` will use `[[` *var* `=~` *regex* `]]` if available and fall back to
`grep -E` otherwise. But the use of bug/feature IDs is not restricted; any
modernish script can query them in the same way.
...
Non-fatal shell bugs and quirks currently tested for are:
...
newline, leaving the shell without a convenient way to refer to control
...
`countfiles`: Count the files in a directory. (It's amazing how many
pitfalls this has, so a library function is needed to do it robustly.)
...
so much faster for small text files.

### use sys/which ###
The `which` command is not standardised, has different options on different
systems and is absent on others. This module provides a cross-platform
consistent version of 'which' written in modernish.
