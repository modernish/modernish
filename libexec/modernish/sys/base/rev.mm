...
( *[Uu][Tt][Ff]8* | *[Uu][Tt][Ff]-8* )
	...
	if str id "$(putln 'mĳn δéjà_вю' | PATH=$DEFPATH command sed "${_Msh_rev_sedscript}")" 'юв_àjéδ nĳm'; then
		_Msh_rev_sed=$(PATH=$DEFPATH; command -v sed)
	elif str id "$(putln 'mĳn δéjà_вю' | command gsed "${_Msh_rev_sedscript}")" 'юв_àjéδ nĳm'; then
